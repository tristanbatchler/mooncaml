...
let init_db_and_start_server port =
  ...
  match Mooncaml_server_db.Util.create_pool db_config with
  | Error (`Connection_error err_msg) ->
    Logs.err (fun m -> m "FATAL: Failed to connect to database: %s" (Caqti_error.show err_msg));
